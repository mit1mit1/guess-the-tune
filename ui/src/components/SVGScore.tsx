...
const ExtraStaveLines = ({ pitch, index }: ExtraStaveLinesProps) => {
  let trackPitch: Pitch = "C4";
  let buffer = []
  let hitEnd = false;
  let newIndex;
  ...
  while (cScale.indexOf(effectivePitch) <= cScale.indexOf(trackPitch) && hitEnd === false) {
    buffer.push(<path key={effectivePitch} strokeWidth="1" stroke="black" d={`M${500 + index * distanceBetweenNotes - 110} ${getHeight(trackPitch)} H ${500 + index * distanceBetweenNotes + 40}`} />);
...
const NotePath = ({ note, index, SVGWidth, SVGHeight, setSelectedNote, color }: NotePathProps) => {
  return (
    <>
      {drawLineUp(note.duration) && <path strokeWidth="4" stroke={color} d={`M${500 + index * distanceBetweenNotes + 2} ${getHeight(note.pitch) - 200} V ${getHeight(note.pitch)}`} />}
      <circle onClick={() => setSelectedNote(index)} cx={500 + index * distanceBetweenNotes - 38} cy={getHeight(note.pitch)} r="40" stroke={color} strokeWidth="3" fill={fillInCircle(note.duration) ? color : "white"} />
      {shouldAddDot(note.duration) && <circle cx={500 + index * distanceBetweenNotes + 19} cy={getHeight(note.pitch)} r="5" stroke={color} strokeWidth="3" fill={color} />}
      {shouldAddEigthLine(note.duration) && <path strokeWidth="16" stroke={color} d={`M${500 + index * distanceBetweenNotes + 2} ${getHeight(note.pitch) - 200 + 8} H ${500 + index * distanceBetweenNotes + 2 + 80}`} />}
      {shouldAddSixteenthLine(note.duration) && <path strokeWidth="16" stroke={color} d={`M${500 + index * distanceBetweenNotes + 2} ${getHeight(note.pitch) - 200 + 8 + 30} H ${500 + index * distanceBetweenNotes + 2 + 80}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke={color} d={`M${500 + index * distanceBetweenNotes - 90} ${getHeight(note.pitch) - 50 + 8 + 30} H ${500 + index * distanceBetweenNotes - 150}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke={color} d={`M${500 + index * distanceBetweenNotes - 90} ${getHeight(note.pitch) - 50 + 8 + 60} H ${500 + index * distanceBetweenNotes - 150 }`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke={color} d={`M${500 + index * distanceBetweenNotes - 110} ${getHeight(note.pitch) - 50 + 8 + 10} V ${getHeight(note.pitch) - 50 + 8 + 80}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke={color} d={`M${500 + index * distanceBetweenNotes - 130} ${getHeight(note.pitch) - 50 + 8 + 10} V ${getHeight(note.pitch) - 50 + 8 + 80}`} />}
  ...
  return (
    ...
    >
      ...
      {incorrectPitches.map((pitchArray, positionIndex) => {
        return pitchArray.map((pitch) => <NotePath setSelectedNote={setSelectedNote} note={{pitch: pitch, duration: "1n."}} index={positionIndex} SVGWidth={SVGWidth} SVGHeight={SVGHeight} color="grey" key={`${positionIndex}-${pitch}`}/>)
      })}
      {guessedNotes.map((note, index) => <NotePath setSelectedNote={setSelectedNote} note={note} index={index} SVGWidth={SVGWidth} SVGHeight={SVGHeight} color="black" key={`${index}-${note.pitch}-${note.duration}`} />)}
    </svg>
  )
}
