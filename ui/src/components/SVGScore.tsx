...
const ExtraStaveLines = ({ pitch, index}: ExtraStaveLinesProps) => {
  ...
  while (cScale.indexOf(effectivePitch) <= cScale.indexOf(trackPitch) && hitEnd === false) {
    buffer.push(<path strokeWidth="1" stroke="black" d={`M${500 + index * distanceBetweenNotes - 110} ${getHeight(trackPitch)} H ${500 + index * distanceBetweenNotes + 40}`} />);
...
const NotePath = ({ note, index, SVGWidth, SVGHeight, setSelectedNote }: NotePathProps) => {
  return (
    <>
      ...
      <circle onClick={() => setSelectedNote(index)} cx={500 + index * distanceBetweenNotes - 38} cy={getHeight(note.pitch)} r="40" stroke="black" strokeWidth="3" fill={fillInCircle(note.duration) ? "black" : "white"} />
      {shouldAddDot(note.duration) && <circle cx={500 + index * distanceBetweenNotes + 19} cy={getHeight(note.pitch)} r="5" stroke="black" strokeWidth="3" fill="black" />}
      {shouldAddEigthLine(note.duration) && <path strokeWidth="16" stroke="black" d={`M${500 + index * distanceBetweenNotes + 2} ${getHeight(note.pitch) - 200 + 8} H ${500 + index * distanceBetweenNotes + 2 + 80}`} />}
      {shouldAddSixteenthLine(note.duration) && <path strokeWidth="16" stroke="black" d={`M${500 + index * distanceBetweenNotes + 2} ${getHeight(note.pitch) - 200 + 8 + 30} H ${500 + index * distanceBetweenNotes + 2 + 80}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke="black" d={`M${500 + index * distanceBetweenNotes - 90} ${getHeight(note.pitch) - 50 + 8 + 30} H ${500 + index * distanceBetweenNotes - 150}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke="black" d={`M${500 + index * distanceBetweenNotes - 90} ${getHeight(note.pitch) - 50 + 8 + 60} H ${500 + index * distanceBetweenNotes - 150 }`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke="black" d={`M${500 + index * distanceBetweenNotes - 110} ${getHeight(note.pitch) - 50 + 8 + 10} V ${getHeight(note.pitch) - 50 + 8 + 80}`} />}
      {shouldAddSharp(note.pitch) && <path strokeWidth="12" stroke="black" d={`M${500 + index * distanceBetweenNotes - 130} ${getHeight(note.pitch) - 50 + 8 + 10} V ${getHeight(note.pitch) - 50 + 8 + 80}`} />}
